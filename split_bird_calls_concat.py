...
def extract_background_segments_with_concatenation(audio_path, output_folder, call_intervals, 
    ...
    for interval_start, interval_end in background_intervals:
        ...
        step_size = min_segment_length  # Step by minimum segment length
    ...
    for bg_clip_num in range(num_background_clips):
        ...
        filename = os.path.join(output_folder, f"background_concat_{bg_clip_num+1}.wav")
...
if __name__ == "__main__":
    ...
    # General parameters
    clip_duration = 3.0  # Duration for bird call clips
    desired_background_duration = 1.0  # Desired duration for background clips
    min_segment_length = 0.2  # Minimum length for background segments to consider
    sr = 22050
    ...
    # Parameters to reduce call overlap
    min_peak_distance = 1.5  # Minimum time between detected calls in seconds
    height_percentile = 75  # Only keep peaks above this percentile (0-100)
    ...
    # Extract bird call segments with reduced overlap
    ...
    # Use the concatenation approach to extract background segments
